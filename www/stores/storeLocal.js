...
const MyStoreLocal = {
  ...
  state: {
    ...
    loginOptionsMobile: {
      // same as loginOptions (s. above) but for mobile view
      dateCache: 0,
      favoriteIdMap: {},
      fieldIdMap: {},
    },
  ...
  mutations: {
    ...
    loginFavorites(state, payload) {
      if (payload.dateCache === state.loginFavorites.dateCache) return;
    ...
    loginOption(state, payload) {
      const getOptions = (obj, fieldId) =>
        obj[fieldId] === undefined
          ? {}
          : JSON.parse(JSON.stringify(obj[fieldId]));
      ...
      if (
        JSON.stringify(options) === JSON.stringify(getOptions(target, fieldId))
      )
        return;
    ...
    loginOptions(state, payload) {
      const base = payload.isMobile
        ? state.loginOptionsMobile
        : state.loginOptions;
      for (const o of payload.options) {
        if (
          o.favoriteId !== null &&
          base.favoriteIdMap[o.favoriteId] === undefined
        )
  ...
  getters: {
    customLogo: (state) =>
      !state.activated || state.companyLogo === ""
        ? "images/logo.png"
        : `data:image;base64,${state.companyLogo}`,

    customLogoUrl: (state) =>
      !state.activated || state.companyLogoUrl === ""
        ? "https://rei3.de/"
        : state.companyLogoUrl,

    loginBackground: (state) =>
      state.companyLoginImage === ""
